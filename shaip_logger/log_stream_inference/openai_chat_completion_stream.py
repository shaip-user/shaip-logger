...
class LogOpenAiChatCompletionStreamInference(LogStreamInference, ShaipApiKey):
    def __init__(
        self,
        prompt_slug: str,
        ...
        try:
            for stream_chunk in response:
                if isinstance(stream_chunk, dict):
                    self.response += self._get_text_from_stream_chunk(stream_chunk)
                else:
    ...
    def log_stream_inference(self):
        """
        logs the stream inference to the shaip api server
        """
        try:
            ...
            payload = {
                ...
                'customer_id': (
                    str(self.customer_id) if self.customer_id is not None else None
                ),
                'customer_user_id': (
                    str(self.customer_user_id)
                ...
                'user_query': (
                    str(self.user_query) if self.user_query is not None else None
                ),
                'external_reference_id': (
                    str(self.external_reference_id)
