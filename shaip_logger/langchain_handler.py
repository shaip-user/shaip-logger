...
class CallbackHandler(BaseCallbackHandler, ShaipApiKey):
    """
        ...
        **kwargs: Any,
        ...
        try:
            ...
            for i in range(len(response.generations)):
                ...
                token_usage = self._get_llm_usage(
                    llm_output=llm_output,
                    prompt=run_info['prompt'],
                    response=response_text,
                    language_model_id=run_info['language_model_id'],
                    is_chat_model=run_info['is_chat_model'],
                )
    ...
    def _get_llm_usage(
        self, llm_output: Dict, prompt, response, language_model_id, is_chat_model
    ) -> Dict:
        """
        ...
        if llm_output is not None and 'token_usage' in llm_output:
            return {
                'prompt_tokens': (
                    ...
                    if 'prompt_tokens' in llm_output['token_usage']
                    else None
                ),
                'completion_tokens': (
                    llm_output['token_usage']['completion_tokens']
                    if 'completion_tokens' in llm_output['token_usage']
                    else None
                ),
                'total_tokens': (
                    llm_output['token_usage']['total_tokens']
                    if 'total_tokens' in llm_output['token_usage']
                    else None
                ),
            }
        else:
    ...
    def _get_prompt_tokens_chat_model(
        self, prompt: List[Dict[str, Any]], language_model_id: str
    ):
        """
    ...
    def _log_llm_response(self, run_info: Dict):
        """
        Logs the LLM response to shaip
        """
        try:
            InferenceLogger.log_inference(
                prompt_slug=run_info['prompt_slug'],
                prompt=run_info['prompt'],
                ...
                response_time=run_info['response_time'],
                ...
                external_reference_id=run_info['external_reference_id'],
                custom_attributes=run_info['custom_attributes'],
            )
