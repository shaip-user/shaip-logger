...
from shaip_logger.tracing.trace import Trace
from shaip_logger.util.extract_model import _extract_model_name


class LangchainCallbackHandler(BaseCallbackHandler, ShaipApiKey):
    def __init__(
        self,
    ...
    def on_llm_new_token(
        ...
        **kwargs: Any,
        ...
        _debug(f"on llm new token: run_id: {run_id} parent_run_id: {parent_run_id}")

    def _get_shaip_run_name(self, serialized: Dict[str, Any], **kwargs: Any) -> str:
        """
        ...
        Returns:
            str: The determined shaip run name for the entity.
        """
    ...
    def on_chain_start(
        ...
        **kwargs: Any,
        ...
        name = self._get_shaip_run_name(serialized, **kwargs)
        ...
        if parent_run_id is None:
            self.runs[run_id] = self.trace.create_span(
                name=name, input=inputs, version=self.version
            )
    ...
    def convert_outputs_to_dict(self, outputs: Dict[str, Any]) -> Dict:
        ...
        if type(outputs) == AgentActionMessageLog:
            outputs = {"tool_input": outputs.tool_input, "log": outputs.log}
        ...
        if type(outputs) == list and all(
            isinstance(item, AIMessageChunk) for item in outputs
        ):
            outputs = [{"type": ai_message_chunk.type} for ai_message_chunk in outputs]
    ...
    def on_chain_end(
        ...
        **kwargs: Any,
        ...
        try:
            _debug(f"on chain end: run_id: {run_id} parent_run_id: {parent_run_id}")
    ...
    def on_chain_error(
        ...
        **kwargs: Any,
        ...
        try:
            _debug(f"on chain error: run_id: {run_id} parent_run_id: {parent_run_id}")
    ...
    def on_agent_action(
        ...
        **kwargs: Any,
        ...
        """Run on agent action."""
        try:
            _debug(f"on agent action: run_id: {run_id} parent_run_id: {parent_run_id}")
    ...
    def on_agent_finish(
        ...
        **kwargs: Any,
        ...
        try:
            _debug(f"on agent finish: run_id: {run_id} parent_run_id: {parent_run_id}")
            if run_id not in self.runs:
                raise Exception("run not found")
    ...
    def on_retriever_end(
        ...
        **kwargs: Any,
        ...
        try:
            _debug(f"on retriever end: run_id: {run_id} parent_run_id: {parent_run_id}")
    ...
    def on_tool_start(
        ...
        **kwargs: Any,
        ...
        try:
            _debug(f"on tool start: run_id: {run_id} parent_run_id: {parent_run_id}")
            if parent_run_id is None or parent_run_id not in self.runs:
                raise Exception("parent run not found")
            self.runs[run_id] = self.runs[parent_run_id].create_span(
                name=self._get_shaip_run_name(serialized, **kwargs),
    ...
    def on_tool_end(
        ...
        **kwargs: Any,
        ...
        try:
            _debug(f"on tool end: run_id: {run_id} parent_run_id: {parent_run_id}")
            if run_id is None or run_id not in self.runs:
                raise Exception("run not found")
    ...
    def on_tool_error(
        ...
        **kwargs: Any,
        ...
        try:
            _debug(f"on tool error: run_id: {run_id} parent_run_id: {parent_run_id}")
            if run_id is None or run_id not in self.runs:
                raise Exception("run not found")
            self._update_run(run_id, None, error)
            self.runs[run_id].end()
    ...
    def on_llm_start(
        ...
        **kwargs: Any,
        ...
        try:
            _debug(f"on llm start: run_id: {run_id} parent_run_id: {parent_run_id}")
            _debug("metadata:")
            _debug(metadata)
            self._on_llm_action(
                serialized,
        ...
        try:
            ...
            if run_id not in self.runs:
                ...
                self.runs[run_id].update(
                    ...
                    completion_tokens=(
                        llm_usage["completion_tokens"] if llm_usage else None
                    ),
    ...
    def on_llm_error(
        ...
        **kwargs: Any,
        ...
        try:
            _debug(f"on llm error: run_id: {run_id} parent_run_id: {parent_run_id}")
            self._update_run(run_id, None, error)
            self.runs[run_id].end()
    ...
    def _on_llm_action(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            prompt_slug = metadata.get('prompt_slug', None)
            _debug(
                f"Creating generation with name: {name} and attributes: {attributes}"
            )
            if parent_run_id in self.runs:
                self.runs[run_id] = self.runs[parent_run_id].create_generation(
                    name=name,
                ...
                self.runs[run_id] = self.trace.create_generation(
                    name=name,
                    attributes=attributes,
                    version=self.version,
                    prompt_slug=prompt_slug,
    ...
    def _generate_trace(
        ...
        **kwargs: Any,
        ...
        try:
            class_name = self._get_shaip_run_name(serialized, **kwargs)
    ...
    def _update_run(self, run_id: str, output: Dict, error: Optional[Exception] = None):
        ...
        if self.trace is not None and self.runs[run_id] is not None:
            if error is not None:
                self.runs[run_id].update(
                    status="ERROR", attributes={"status_message": str(error)}
                )
            else:
