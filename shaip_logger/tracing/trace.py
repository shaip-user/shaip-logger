...
from shaip_logger.constants import API_BASE_URL
from shaip_logger.request_helper import RequestHelper
from langchain.schema.document import Document


class Trace(ShaipApiKey):
    _trace: TraceModel
    _spans: list[Span]
    ...
    def __init__(
        ...
        self._trace = TraceModel(
            ...
            start_time=(
                start_time
                or datetime.datetime.utcnow().replace(tzinfo=datetime.timezone.utc)
            ).isoformat(),
    ...
    def update(
        ...
        if end_time:
            self._trace.end_time = end_time.replace(
    ...
    async def _log_trace_async(self, request_dict: Dict[str, Any]):
        RequestHelper.make_post_request(
    ...
    def end(self, end_time: Optional[datetime.datetime] = None):
        try:
            ...
            if self._trace.duration is None:
                delta = end_time - get_utc_time(
                    datetime.datetime.fromisoformat(self._trace.start_time)
                )
                self._trace.duration = int(
                    (delta.seconds * 1000) + (delta.microseconds // 1000)
                )
            request_dict = remove_none_values(self.to_dict())
            threading.Thread(
                target=lambda: asyncio.run(self._log_trace_async(request_dict))
            ).start()
