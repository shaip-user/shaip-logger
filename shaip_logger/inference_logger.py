...
class InferenceLogger(ShaipApiKey):
        ...
        try:
            ...
            threading.Thread(
                target=lambda: asyncio.run(
                    InferenceLogger._log_inference_asynchronously(*args)
                )
            ).start()
        except Exception as e:
            print("Error in logging inference to Shaip: ", str(e))
    ...
    async def _log_inference_asynchronously(
        ...
        """
        logs the llm inference to shaip
        """
        try:
            payload = {
                'prompt': prompt,
                ...
                'customer_user_id': (
                    str(customer_user_id) if customer_user_id is not None else None
                ),
